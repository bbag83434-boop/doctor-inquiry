...
export function DoctorDetailsScreen() {
  ...
  return <main className="doctor-details" aria-label={`${doctor.name} profile`}>
    ...
    <div className="details-booking"><div><span>{doctor.availability}</span><strong>Book a consultation</strong></div><Button fullWidth onClick={() => navigate(`/doctors/${doctor.id}/book`)}>Book Appointment</Button></div>
  </main>;
}
